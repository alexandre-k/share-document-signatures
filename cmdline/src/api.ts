...
export const generateSignatureRequestData = ({ signers, clientId, title, subject, message, ccEmailAddresses, fileUrl, testMode }: Api.IGenerateSignatureRequestData): HelloSignSDK.SignatureRequestSendRequest => {
...
export const sendSignatureRequest = async ({ signers, clientId, title, subject, message, fileUrl, testMode}: Api.ISendSignatureRequest): Promise<HelloSignSDK.SignatureRequestGetResponse> => {
    const api = new HelloSignSDK.SignatureRequestApi();
    api.username = process.env.HELLOSIGN_API_KEY || "undefined";
